...
@pytest.mark.positive
def test_segments_create_flow(page, base_url):
    segments = pages.SEGMENTS.create(page, base_url)
    segments.open()
    segments.expect_heading()
    segments.expect_primary_action_enabled()


@pytest.mark.negative
def test_regions_block_empty_name(page, base_url):
    regions = pages.REGIONS.create(page, base_url)
    regions.open_with_query([("name", "")])
    regions.expect_heading()
    regions.expect_primary_action_disabled()


@pytest.mark.positive
def test_tags_create_flow(page, base_url):
    tags = pages.TAGS.create(page, base_url)
    tags.open()
    tags.expect_heading()
    tags.expect_primary_action_enabled()


@pytest.mark.negative
def test_environments_require_owner(page, base_url):
    environments = pages.ENVIRONMENTS.create(page, base_url)
    environments.open_with_query([("owner", "")])
    environments.expect_heading()
    environments.expect_primary_action_disabled()


@pytest.mark.positive
def test_playbooks_create_flow(page, base_url):
    playbooks = pages.PLAYBOOKS.create(page, base_url)
    playbooks.open()
    playbooks.expect_heading()
    playbooks.expect_primary_action_enabled()
